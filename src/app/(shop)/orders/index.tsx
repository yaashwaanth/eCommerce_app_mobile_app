...
import { getMyOders } from '../../../api/api'

const renderItem:ListRenderItem<Tables<'orders'>> = ({item}) => <Link href={`/orders/${item.slug}`} asChild>
...
const Orders = () => {
  ...
  const {data: orders,error, isLoading} = getMyOders();
  if(isLoading) return <ActivityIndicator/>

  if(error || !orders) return <Text>Error : {error?.message}</Text>

  if(!orders.length) return <Text style={{fontSize:16,color:'#555',textAlign:'center',padding:10}}>No orders placed yet</Text>
